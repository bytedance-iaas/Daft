...
def container_findings(input_format: str, info: dict, robot: dict,
    ...
    if input_format == "rrd":
        ...
        # lance/mcap(2026-09-18):留痕口径与 RRD 同族但字段更少 —— 两种格式都没有
        # 溯源约定,型号/时间/标注三项该说清"文件自带还是人工补的"。
        _carrier = ("schema metadata" if input_format == "lance" else "metadata 记录")
        rt_src = str(info.get("robot_type_source") or "")
        file_rt = str(info.get("robot_type_file") or "")
        if registry_hit and emb != "unknown":
            if rt_src == "embedded":
        ...
        if ts == "timestamps":
            out.append({"项": "帧时间信息", "状态": "正常",
                        "说明": "表内 timestamp 列自带时间轴,全自动。"})
        elif ts == "log_time":
            out.append({"项": "帧时间信息", "状态": "正常",
                        "说明": "消息自带 log_time,时间轴取自 action topic,全自动。"})
        elif ts == "metadata":
            out.append({"项": "帧时间信息", "状态": "正常",
                        "说明": f"{_carrier}带 fps={info.get('fps')},按恒定帧率换算时间轴。"})
        elif ts == "config":
            out.append({"项": "帧时间信息", "状态": "缺失(已补)",
                        "说明": f"数据内没有时间信息,已按人工指定的 ingest.lance_fps="
                                f"{info.get('fps')} 换算时间轴。该值若与真实采集帧率"
                                "不符,时序/同步类结论会整体失真 —— 请核对。"})
