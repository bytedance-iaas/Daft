...
def build_console_app(delivery: str, config_path: str | None = None,
    ...
    首屏要渲染几百个组件,前端引导 10-14 秒,深链预填要等它全部完成才出现 ——
    用户以为功能不存在。实测服务端五个启动回调只要 0.07s,时间全耗在前端;
    组件数是主导因子,所以按"操作面/阅读面"切成两个应用:
      · 本函数 = 首页(Diagnose 深链落点,组件几十个,秒开);
      · build_app = 质检报告(挂 {root}/reports,谁看报告谁买单)。
    两边不共享任何组件与回调;跳转是普通链接。
    """
    import gradio as gr
...
def build_app(delivery: str, config_path: str | None = None, probe_timeout: float = 5.0,
    ...
    2026-08-19 拆分(治首屏,分工见 build_console_app 的 docstring):任务台与
    终端搬去了轻量的 build_console_app,本函数只剩报告全家桶(质检总览/
    Episodes/人工裁决/技能画像/明细/性能剖析…),由 create_asgi_app 挂在
    `{root}/reports` —— 组件几百个,谁看报告谁买单,不再拖累首页。
    报告页那套子页签的顺序、默认落地页、组件与回调仍一律不动(用户红线)。

    review_dir = 审片站根目录(与 `/review` 静态路由同一个),Episodes 页的视频
    来源链第一档指着它;不给就只剩交付集内的视频。
    console_href = 返回任务台的链接(create_asgi_app 传绝对前缀;不给就不渲染)。
    ...
    with gr.Blocks(title="Robot Data Curation") as app:
        ...
        if console_href:
            # 回任务台的普通链接(2026-08-19 拆分后两个子应用间只靠链接往来)
            gr.Markdown(f"[← 返回任务台]({console_href})", elem_id="console-link")
        with contextlib.ExitStack() as shell:
            # 顶层导航壳保留(#topnav 的样式/CSS 选择器都锚着它),但本应用里
            # 只剩「质检报告」一个页签 —— 任务台与终端 2026-08-19 搬去了
            # build_console_app,报告页子页签的顺序与内容一个字没动。
...
def create_asgi_app(delivery: str, config_path: str | None = None,
    ...
    # 首页 head 追加报告预热脚本(reports_prefetch_head):首页秒开,报告的
    # JS/config 在空闲时灌进浏览器缓存,点「查看质检报告」时下载成本已付讫。
    pres = presentation(terminal, root)
    pres["head"] += reports_prefetch_head(f"{root}/reports/" if root
                                          else "/reports/")
