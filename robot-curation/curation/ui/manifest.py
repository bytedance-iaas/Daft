...
def sync_conclusion(m: dict) -> dict:
    ...
    if n_noisy:
        points.append(
            f"**{n_noisy} 条**的某一路互相关峰偏离了 0,但**赢不过 0**"
            "(零滞后处的相关与峰值相当)——这是背景有人走动、相机晃动或画面里"
            "机械臂占比小造成的**假峰**,证据其实偏向对齐,不是错位。"
            "逐条曲线右侧的诊断框写明了是哪一路、该怎么改善。")
    if n_suspect:
        level = "notice" if level == "ok" else level
        points.append(
            f"**{n_suspect} 条**有相机**疑似错位但证据不足**(互相关峰明显偏离 0,"
            "但峰形不够硬,可能是背景有人走动/相机晃动造成的假峰)。系统**不判废也不"
            "进人工队列**,但这类条目值得抽查:如果同一路反复出现,多半是真延时。")
    if n_undec:
