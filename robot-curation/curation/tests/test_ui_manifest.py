...
def test_console_and_reports_are_separate_apps(delivery):
    """2026-08-19 拆分契约:任务台与报告是**两个独立应用**,互不携带对方的
    组件 —— 首屏(任务台)的组件量就是它快的原因,报告页签一旦回流首页,
    首屏引导时间立刻回到 10 秒时代。报告页那套子页签一个不少(红线照旧)。
    """
    pytest.importorskip("gradio")
    from curation.ui.app import build_app, build_console_app
    con = _config_text(build_console_app(delivery))
    for t in ("跑质检", "执行人工裁决", "任务与日志"):
        assert t in con, t
...
def test_asgi_app_serves_console_home_and_reports_subpath(delivery, clean_ui_env):
    """双挂载(2026-08-19 拆分):`/` = 轻量任务台(含跑质检表单),
    `/reports/` = 报告应用(含同步曲线页签)。两个都要 200 且内容各归各。"""
    pytest.importorskip("gradio")
    from starlette.testclient import TestClient
    ...
    with TestClient(app) as c:
        ...
        assert "数据集 TOS 路径" in home.text
        # 报告组件不进首页(「视频-动作同步」不能当判据 —— 它同时是任务台
        # 「要跑的模块」勾选组里的模块名;用报告页独有的筛选文案)
        assert "只看有标注/异常的" not in home.text
        rep = c.get("/reports/")
        assert rep.status_code == 200
        assert "视频-动作同步" in rep.text and "只看有标注/异常的" in rep.text
