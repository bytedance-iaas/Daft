...
两个基线的源格式如果都是 LeRobot v2，v3 的读取和导出路径（多条拼接的 parquet / mp4、按 chunk 重编码）就没有被覆盖。
需要一个小的 v3 数据集补上这一块，哪怕只有几十条 —— 待需求方指认；没有现成的，就用 lerobot 官方工具把
droid 前 50 条转一份 v3。确定性六项要查到有没有字节级重复的条目，没有的话去重这一行也是空转，
同样靠夹具补（复制两条 episode 进迷你数据集）。
