...
|---|---|---|---|---|---|
| `probe` 任务完成度打分 | 0.5s 间隔、最长边 448、≤4 机位 | 各机位按下标对齐成「时刻」，linspace 含首尾取 8 个时刻，**打乱顺序** | 2×机位数：各机位第 0 帧作参考 + 该时刻各机位画面；模型回 0–100 一个整数 | 8 | 过了两道硬门的每一条 |
...
- probe **一个时刻一次请求且打乱顺序**：让模型看不到时序，只按物体状态打分。
...
- 任务级汇总 = 所有桶求和，**含重试和子任务花掉的**，展示在任务详情和报告里，可按子任务展开。
  **不换算金额**（D12）。
...
外层重试用尽仍失败的那条 episode，该模块的结果行是 `verdict=error`（task_success 里表现为
「VLM 调用/解析失败」的弃权）；「重试」默认只重跑这些条目（03 篇 §3.2），对应 v1 的 `--retry-abstained`。
...
调优通过站点配置和 planner 进行，不需要、也没有对外的自定义提交入口。
