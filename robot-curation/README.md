...
自选模块里，「要跑的模块」框里其余模块是勾选框，配合「只跑选中 / 跳过选中」使用；**运动学极限**是同一个框里的最后一行，是一个机器人型号下拉：

- 选项只有注册表里有规格表的型号（`curation/registry/profiles/*.yaml`，目前是 agibot、aloha、franka、google_robot、pusht、so100、so101、ur5、widowx），外加「不检查」
- 选型号 = 检查运动学极限，并按这个型号的规格查（覆盖数据集里声明的型号和「更多设置」里填的型号）；选「不检查」= 跳过这一项
- 它不受「只跑选中 / 跳过选中」影响；「只跑选中」时一个框都不勾、只选型号，就是只跑运动学极限

手动验证：选「自选模块」→ 勾「精确去重」、保持「只跑选中」→ 运动学极限选 `so101` → 开始质检，这次任务的 `<交付根>/.runs/<任务编号>/cmd.json` 里 `argv` 应带 `--only dedup,kinematic_limits --embodiment-id so101`；改选「不检查」再跑，`argv` 只剩 `--only dedup`，且即使数据集没登记型号也不会弹「机器人型号」追问框。
