...
Web 质检台的「质检范围」三选一：完整质检、快速质检（不跑要调模型的任务成败判定与技能画像）、自选模块。

自选模块里，「要跑的模块」框里其余模块是勾选框，配合「只跑选中 / 跳过选中」使用；框里最后一行是「□ 运动学极限」加一个机器人型号下拉：

- 不勾：下拉置灰，不检查运动学极限
- 勾上：下拉亮起，从注册表里有规格表的型号里选（`curation/registry/profiles/*.yaml`，目前是 agibot、aloha、franka、google_robot、pusht、so100、so101、ur5、widowx），按这个型号的规格查（覆盖数据集里声明的型号和「更多设置」里填的型号）；勾上但没选型号时，用「更多设置」或数据集里声明的型号，都没有就在开跑前弹「机器人型号」追问
- 这一行不受「只跑选中 / 跳过选中」影响；「只跑选中」时别的都不勾、只勾运动学极限，就是只跑这一项

手动验证：选「自选模块」→ 型号下拉应为灰色 → 勾「精确去重」、保持「只跑选中」→ 勾「运动学极限」，下拉亮起，选 `so101` → 开始质检，这次任务的 `<交付根>/.runs/<任务编号>/cmd.json` 里 `argv` 应带 `--only dedup,kinematic_limits --embodiment-id so101`；取消勾选「运动学极限」（下拉变灰）再跑，`argv` 只剩 `--only dedup`，且即使数据集没登记型号也不会弹「机器人型号」追问框。
