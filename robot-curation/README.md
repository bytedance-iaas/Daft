...
运动学极限要按机器人型号查规格表（`curation/registry/profiles/*.yaml`，目前支持 agibot、aloha、franka、google_robot、pusht、so100、so101、ur5、widowx）。这次要跑运动学极限时，型号按 `info.json` 的 `robot_type` 来：

- 读到了、规格库支持：正常检查
- 读到了、规格库不支持：运动学极限**整项跳过**，其余模块照常，任务不会因此失败；日志打一行警告，报告「数据包完整性」里注明
- 读不到：Web 质检台开跑前弹「机器人型号」追问框（命令行在终端里问）；选了型号就按它查，选跳过就不查。没被问到的情况（「跑根目录下的全部数据集」、非交互命令行）同样整项跳过
- 没勾运动学极限：不看型号，也不追问

手动验证：拿一个 `robot_type` 是规格库里没有的型号（如 `koch`）的数据集，「完整质检」或自选模块勾上「运动学极限」开跑 → 不弹追问，任务正常跑完，日志有「机器人型号 koch 不在规格库…运动学极限整项跳过,其余模块照常」；把 `robot_type` 删掉再跑 → 开跑前弹「机器人型号」追问框；自选模块不勾「运动学极限」再跑 → 不弹追问。
