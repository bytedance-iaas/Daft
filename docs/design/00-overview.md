...
> Curator v2 工程设计文档。本册共 13 篇，本篇是入口：讲清楚分层、边界、数据流和本期范围。
...
- **单副本**。任务在 Daemon 进程内的 worker 池里排队执行，1000 条还是 10000 条 episode
  都打到同一个节点上排队。**同一时间默认跑 3 个任务**，其余排队，可调（Daemon 的 `CURATOR_MAX_RUNNING_TASKS`）；
  几个任务的 CPU 档共用 Daemon 管的一个全局 CPU 池（D54），见 `04-concurrency-and-vlm-merge.md` §2.3。
...
例外是已被正常判完的模块确定拒绝的条目（D35）：出错模块重跑出什么结果都翻不了案，所以直接拒绝、不进 `held`，
出错的模块照样记在它身上。
...
```
重试        只补跑 held 里的 episode：从它出错的那一档接着往后跑，已成功的结果原样保留
...
子任务结束后，父任务的终态按**当前**结果重算：补跑把出错的条目都救回来了，「错误」就变成「已完成」；
...
| D25 | 重试只补跑出错的 episode，成功的结果保留；新报告完整生成后才替换旧报告；补跑后任务终态按当前结果重算（错误 → 已完成），历史留在执行时间线 |
...
| D28 | 删除任务只处理平台里的记录，不动 TOS；「清理交付产物」是单独的动作，展示路径并二次确认。2026-09-23 补充（需求方第二轮第 19 条）：删除确认框另有一个默认不勾的「同时清理交付产物」，勾上时照样写出要删的路径，先清理、成功后才删除任务；不勾时与原来一样 |
...
| D35 | 已被正常判完的模块确定拒绝的条目（某个硬门确定失败，或所有已勾选的软分模块都给了分、加权分低于阈值），即使另有模块对它执行出错，也直接拒绝、不进待补跑；出错的模块照样记在该条目上。重试只补跑待补跑的条目；任务终态看有没有整体失败的模块和待补跑的条目（细化 D24、D25、D33） |
| D36 | 导航增加「概览」「数据集」两页。数据集是平台里登记的实体：添加时做预检，并记下 meta 指纹和全量文件清单指纹；质检任务从已添加的数据集里选（新建任务时当场添加的也会登记进来） |
| D37 | 开始质检任务时核对数据集指纹：与添加时（或最近一次预检时）不一致，就弹框说明变了什么、请用户确认，确认后重新预检；预检结果与任务配置相容则接着开始，不相容则回到编辑页。运行中再发现变化仍按 D27 处理 |
| D38 | 新建任务分两屏：第一屏是基本且必填的输入（同静态稿）；第二屏针对已开启的模块，给出各自需要的额外输入（必填 / 选填，由模块注册表的参数 schema 驱动），然后创建 |
| D39 | 人工改标后重判任务成败，默认照 v1 的 `rejudge`：只跑多视角打分和逐机位复核两层（不跑任务类型判定、机位提示、判废护栏和取证仲裁），结论与调用和 v1 一致。执行裁决的对话框说明这一点，并提供「按首轮的完整流程重判」的选项；选了什么记在子任务和每条改标上，重试沿用。人已给出成败结论的条目照旧不重判 |
| D40 | 源文件缺失（parquet 或某路视频不在）的 episode 照 v1 剔除：不质检、不进三份清单、不计入总数；报告列出跳过了哪几条、各缺什么文件，任务汇总给出条数 |
| D41 | 技能画像整个模块失败时维持 P10 + P11：全部条目待补跑、一条都不交付，等「重试」成功。与 v1 不同（v1 从不因画像挡交付）；开始前的 VLM 检查会拦住大部分模型不可用的情况 |
| D42 | 已被拒绝的条目（被去重或任何模块拒掉）不再进人工的成败裁决，问它任务成没成功已经没有意义。去重的拒绝和只归因于任务成败判定的拒绝一样可以复议：列在裁决页的「被拒复议」里，写明与哪条重复，「恢复为可用」即推翻去重的结论、回到通过（它若有成败弃权，下一版里照常进成败裁决）。时间戳、运动学、同步这些物理与结构硬门和软分拒绝仍是终局。与 v1 不同：v1 的重复项仍进成败裁决队列、人判成功就交付，复议只认任务成败判定 |
| D43 | 人工复核的种类由模块注册表声明：注册表带一份复核种类目录（编号、标题、条目所在的清单、是否计为待裁、可选的判断及按钮名），每个模块写明自己产生哪几种、它的拒绝可否复议。契约里种类与判断都是开放字符串，由 Daemon 按目录校验；前端对没有专用视图的种类按目录通用渲染。以后加一种复核 = 目录加一项 + `adjudicate-apply` 加一条执行规则，契约不变 |
| D44 | mcap / lance 两种输入格式全量接入 v2，并支持 TOS 上的数据（先拉到本地缓存再读）：`dev` 上的 PR #155 同步进来，冻结点从 `release_v1@45bdf9292` 前移到 `dev@eb637ba40`（A 类清单与对账随之更新）。取代 D34 里「新的输入格式代码照搬、默认关闭、预检判为不支持」对这两种格式的做法；`.rrd` 维持原样 |
...
| D49 | EEF–视频一致性由建议性的两个模块改为**一个参与判决的模块**（需求方 2026-09-23，取代设计 12 的 D-E3、D-E6 与 §9.4）：`eef_video_review` 并入 `eef_video_consistency`，vlm 档、一票否决、只跑前面没被判废的条目；模块内先 CPU 测量、再请模型复核，每条给出判过、判废或转人工裁决（口径见设计 12 D-E12、D-E13），跑完一定有结论；阈值仍是未校准的 demo profile，模块仍需手动勾选、不进任何预设 |
...
| # | 事项 | 默认 | 落在哪 |
|---|---|---|---|
| P1 | 同时运行的任务数 | 3，其余排队；可调（`CURATOR_MAX_RUNNING_TASKS`）。VLM 并发预算在任务开跑时按运行中的任务数均分；CPU 由 Daemon 的全局 CPU 池在任务之间分（2026-09-25 由 1 改为 3，D54） | 04 §2.3 |
| P2 | 429 / 5xx 自适应降并发放在哪层 | CLI 进程内的 VLM 客户端；Daemon 只给初值、收事件 | 04 §7 |
| P3 | 深链带多个数据集 | 新建页进入批量模式，共用一套配置，一次建 N 个任务 | 07 §2.1 |
| P4 | CPU 档并发 | CLI 默认 1（需求原话）；经 Daemon 时每个 worker 占一个核、单线程，一个任务最多用「容器 CPU 配额 − 2」个（至少 1，留 2 核给 Daemon 与网页），任务可以再往下压；站点不再设 CPU 并发（2026-09-25 取代原来的 `min(8, 核数/4)`，D54） | 04 §2.1 |
...
| 12 | EEF–视频一致性（DEMO 模块，2026-09-22 立项；格式规范与 Schema 在 `docs/contracts/eef/`） | 后端、前端 |
