...
失败但带回了 `usage` 的请求照实记。`requests` 只数拿到了 `usage` 的请求，两者之和才是发出的总数（C3 1.1）。
实际账里一次合并请求记在它携带的各模块 id 排序后用 `+` 连起来的名下（如 `example_grasp+example_table`），
分摊账再拆回单个模块；分摊按整数精确拆分，合计与实际账逐项相等。
