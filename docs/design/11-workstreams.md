...
| **W13** | EEF–视频一致性 DEMO（2026-09-22 立项） | `extensions/eef_consistency/`、注册表 1.4、`trajectory.json` 入口、advisory 分支、报告小节；拆分与验收见 12 篇 §14 与账本 F5.1–F5.7 | W2 + W3 + W4 + W6 已合并 | ✅（第一刀单 agent 串行） |

## 3. 推进批次

```
批次 0（串行，谁也绕不开）
   W0 对账工具 + 黄金基线   ←── 工具最先做；真实数据的基线暂缓，F4.2 黄金对账之前补（见下）
   W1 baseline 清理
   W2 契约冻结
        │
批次 1（大并行）
...
**卡点再放宽**（2026-09-21）：需求方决定暂不生成黄金基线（v1 在现网 Pod 上跑过、结果正常，旧系统有备份）。
W3 拆 v1 编排不再等基线存档，改由合成数据集上的逐位对账守住：冻结点的 v1 和 v2 各跑一遍 `tools/parity` 的
8 条合成数据（假模型，含模型调用图），`compare --all-strict` 必须通过，并进 CI。真实数据的两个基线在
W12 / F4.2 黄金对账之前补上；冻结点源码随时可以提取，晚补不影响基线本身。代价是只在真实数据上才出现的差异要到那时才暴露。

**批次 4（2026-09-22 立项）**：W13 EEF–视频一致性 DEMO。前置全部已合并；只加 B 类代码与新契约（`docs/contracts/eef/`、注册表 1.4），
旧判决与合成数据逐位对账不能变。开工指引在 `12-eef-video-consistency.md` 首节。
