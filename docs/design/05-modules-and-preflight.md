...
复核种类还可以带「后续问题」（`follow_ups`，注册表 1.3）：选了某些判断之后，卡片多出另一条线的问题。现在只有一处，
照搬 v1：标注分歧选了「采纳新标注 / 自行改写标注」，而这条没有成败弃权时，可以顺手给出成败结论（选填：给了机器直接采信、
不再重判；留空则按新标注重判）。打开它的判断一变，这个回答就作废。
...
```
① caption      VLM 读取连续视频生成描述（当前视频协议的 autolabel 缓存可复用）
② 归类文本     每条优先采用视频 caption，不可观察或无描述才退回原始标注，来源留痕
...
2026-09-24 按需求改为视频描述优先，取代 v1 的标注优先规则，唯一实现位于
`grouping_text_and_source`。标注与描述的分歧审计继续独立执行；视频输入细节见
[13 连续视频输入与判定](13-video-native-vlm.md)。
