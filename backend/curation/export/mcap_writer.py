...
def export_mcap_curated(delivery_dir: str, input_dir: str, keep_ids: list[str],
    ...
    records = []
    # 编号稳定性(2026-09-21 审查实锤):reader 对非 episode_N.mcap 的文件按**排序
    # 位置**编号 —— 这种数据集筛选交付后若保留源文件名,交付集再读一遍位置就变了
    # (源 a/b/c 只交付 ep2 的 c.mcap,重读变 ep0),与 index.json 对不上。所以:
