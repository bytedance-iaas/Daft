...
def change_task_state(repo: P.Repository, hub: EventHub | None, task_id: str, frm: Iterable[str],
                      ...
                      publish_done: bool = True) -> bool:
    """CAS ``frm -> to``; False (and nothing written) when the task is not in ``frm``."""
    frm = set(frm)
    with repo.transaction():
        task = repo.get_task(task_id, include_deleted=True)
        ...
        repo.append_event(actor=actor, action="task.state", resource=task_id, at=at,
                          owner=task.owner_id,
                          detail={"from": task.state, "to": to, "pause_reason": new_pause,
                                  "prev_pause_reason": task.pause_reason, "reason": reason,
                                  "by": by})
    if hub is not None:
...
def subtask_pause_reason(repo: P.Repository, sub: P.Subtask, *, scan: int = 500) -> str | None:
    """Subtasks have no ``pause_reason`` column (C5 gap): read it from their audit events."""
    cursor, seen = None, 0
    while seen < scan:
        page = repo.list_events(resource=sub.task_id, cursor=cursor, limit=100)
...
def change_subtask_state(repo: P.Repository, hub: EventHub | None, subtask_id: str,
                         ...
                         by: str = "", publish_done: bool = False) -> bool:
    ...
    with repo.transaction():
        ...
        if sub.state not in frm:
            return False
        prev_pause = subtask_pause_reason(repo, sub) if sub.state in _PAUSING else None
        ...
        task = repo.get_task(sub.task_id, include_deleted=True)
        repo.append_event(actor=actor, action="subtask.state", resource=sub.task_id, at=at,
                          owner=task.owner_id,
                          detail={"subtask_id": subtask_id, "kind": sub.kind, "from": sub.state,
                                  "to": to, "pause_reason": new_pause,
                                  "prev_pause_reason": prev_pause, "reason": reason, "by": by})
    if hub is not None:
...
def record_revision(repo: P.Repository, task_id: str, revision: int, *, at: int,
                    actor: str = "system", subtask_id: str | None = None) -> None:
    """Call after ``switch_result_rev`` succeeded, so the timeline shows the new revision."""
    repo.append_event(actor=actor, action="task.revision", resource=task_id, at=at,
