...
All of them read a committed result revision of the task from its run directory
...
from ..results import episode_list as L
...
def list_task_episodes(request: Request, task_id: str, rev: int | None = Query(None, ge=1),
                       ...
                       limit: int = Query(L.DEFAULT_LIMIT, ge=1, le=L.MAX_LIMIT)):
    rt, task, revision = _task_and_revision(request, task_id, rev)
    return L.page(store_of(rt), rt.repo, task, revision, list_name=list_name, review=review,
