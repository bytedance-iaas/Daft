...
4. **新建任务**：第一屏填 `tos://pai-kit-datasets/lerobot/droid-200`，选访问密钥，停顿后自动预检，模块卡片显示可用 / 需补充 / 不支持（带原因）。交付目录访问密钥选 prod-tos，交付目录填 `tos://pai-kit-scratch/x`，失焦后做写探针：写得进但探针对象删不掉，给橙色警告；交付目录访问密钥换成 old-ci 报红。模型配置默认选中默认模型（模拟世界里是 ark-prod 的 doubao-seed-2-0-pro-260215，下拉里带「（默认）」），可以改；没有默认模型时和以前一样要自己选。第二屏按 param_schema 生成模块参数，运动学极限要选机器人型号或跳过。「创建并开始」：droid-200 开始前指纹不一致，弹框确认后重新预检。深链：`/?dataset=tos://pai-kit-datasets/lerobot/droid_100&region=cn-beijing` 会跳到新建页并预填，每个参数都有常驻提示。
