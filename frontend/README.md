...
4. **新建任务**：第一屏填 `tos://pai-kit-datasets/lerobot/droid-200`，选访问密钥，停顿后自动预检，模块卡片显示可用 / 需补充 / 不支持（带原因）。交付目录访问密钥选 prod-tos，交付目录填 `tos://pai-kit-scratch/x`，失焦后做写探针：写得进但探针对象删不掉，给橙色警告；交付目录访问密钥换成 old-ci 报红。模型配置默认选中默认模型（模拟世界里是 ark-prod 的 doubao-seed-2-0-pro-260215，下拉里带「（默认）」），可以改；没有默认模型时和以前一样要自己选。第二屏按 param_schema 生成模块参数，运动学极限要选机器人型号或跳过。「EEF–视频一致性」卡片是橙色「需要上传约定格式的 trajectory.json」，任何预设与「全选可用」都不勾它（建议性模块只能手动勾）；手动勾上后第二屏有 trajectory.json 与观测种子两个文件项（`format: upload`）：不选文件就保存会提示「请填写trajectory.json」，选一个含 `"truth"` 键的文件会显示定位到样本的错误，选合格文件后显示文件名、sha256 前 12 位与摘要，提交的 `modules[]` 里是 `upload:` 句柄。「创建并开始」：droid-200 开始前指纹不一致，弹框确认后重新预检。深链：`/?dataset=tos://pai-kit-datasets/lerobot/droid_100&region=cn-beijing` 会跳到新建页并预填，每个参数都有常驻提示。
