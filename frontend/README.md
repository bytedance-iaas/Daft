...
设计依据：`docs/design/07-frontend.md`（主规格）、`03-rest-api.md`，契约 `docs/contracts/openapi.yaml`（C4 1.5.1，请求和响应的形状只认它）与 `modules.json`（C1），静态稿在 `mockups/`（只读参考）。
...
| 卡片上的问题分不出是它自己的，还是答了 `follow_ups` 之后多出来的：Daemon 把答过的选填成败结论当作卡片的普通问题发回来，读的时候也不管打开它的回答变没变 | 同一次打开页面时按目录判断；刷新后这一问显示成普通的成败问题，改标的回答改掉以后它还在，直到重新读取（执行时服务端照样不采信作废的回答） |
