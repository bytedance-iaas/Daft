...
export const zh = {
  ...
  taskForm: {
    ...
    episodesAll: (n: number | string) => `全部 ${n} 条`,
    episodesHead: '前 N 条',
    episodesExplicit: '自选',
    headN: '条数',
    ...
    optionalTitle: '可选设置',
    noSettings: (names: string) => `无需额外设置：${names}`,
    skippedTitle: '已跳过',
    skipModule: '跳过该模块',
    unskip: '恢复',
    embodiment: '机器人型号',
  ...
  overview: {
    title: '概览',
    desc: '现在有什么要处理，整体跑得怎么样。',
    todo: '待处理',
    todoNone: '没有要处理的事',
    errorTasks: '错误的任务',
    errorTasksFoot: '可以重试',
    adjudication: '待裁决',
    adjudicationFoot: (tasks: number) => `条，分布在 ${tasks} 个任务`,
    delivery: '交付待导出',
    deliveryFoot: '个任务的交付数据集要（重新）导出',
    datasetsChanged: '数据集有变化',
    datasetsChangedFoot: '个待重新预检',
    keysFailed: '访问密钥验证失败',
    backendsFailed: 'VLM 后端验证失败',
    showTasks: '看是哪些任务',
    running: '运行情况',
    runningCount: '运行中',
    queuedCount: '排队中',
  ...
  taskDetail: {
    ...
    colOps: '操作',
    modulesTitle: '模块',
    modulesDesc: (n: number, skipped: string) => `${n} 个模块参与本次质检${skipped ? `；${skipped}未运行` : ''}`,
    ...
    timeline: '执行时间线',
    timelineDesc: '主流程、子任务、系统暂停都记在这里；每个结果版本都能打开当时的报告',
    timelineKind: {
      created: '创建',
      started: '开始',
      system_pause: '系统暂停',
    ...
    config: '任务配置',
    configDesc: '启动后只能改名称和备注；要换数据、模块、模型或参数，复制为新任务',
    plan: '执行计划',
    planDesc: 'planner 生成，只读；调优改站点配置',
  ...
  sections: {
    ...
    eef: {
      ...
      hypotheses: '被支持的诊断假设',
      review: { reviewed: '完整复核', incomplete: '未完成', not_reviewed: '未复核', needs_human: '待人工看', conflicts: '与 CPU 冲突的窗口', windows: '复核窗口', answered: '有答复', failed: '失败' },
  ...
  credentials: {
    ...
    modelsNotListed: '没能拉到模型列表，不影响使用：直接填 Model ID 或推理接入点 ID',
    refreshModels: '重新拉取',
    refreshed: (n: number) => `拉到 ${n} 个模型`,
    refreshFailed: (note: string) => `这次没拉到模型列表${note ? `：${note}` : ''}。可以手填`,
    addModel: '添加并验证',
    addModelPlaceholder: '搜索已有模型，或填 Model ID、推理接入点 ID（ep-…）添加',
    modelSearchHint: (shown: number, total: number) => `筛出 ${shown} / ${total} 个模型；列表里没有就直接添加`,
