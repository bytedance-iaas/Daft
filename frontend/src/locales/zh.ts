...
export const zh = {
  ...
  summaryKeys: {
    ...
    to_human: '转人工裁决',
    outcomes: '模块结论（条数）',
    human_reasons: '转人工的原因（条数）',
    reject_subitems: '判废来自的分项（条数）',
    model_cpu_agreement: '模型与 CPU 一致率',
    model_votes: '模型明确的票数',
  ...
  taskDetail: {
    ...
    planEstimate: (req: number, wall: string) => `计划估算：VLM 请求约 ${req} 次，耗时约 ${wall}`,
    // the planner writes its gates, merge strategy and estimate notes in English (C2 plan.json):
    // the page says them in Chinese (fourth round)
    planGates: {
      episode: '并行条目',
      probe: '打分',
      endstate: '逐机位复核',
    ...
    planMerge: { none: '请求合并：不合并', per_episode_multi_module: '请求合并：同一条的多个模块合并' } as Record<string, string>,
    planNoteRough: (seconds: string, use: string, since: string) =>
      `粗估：假设选中的条目都通过硬门，每个请求按 ${seconds} 秒、闸门占用 ${use} 计（v1 在 ${since} 实测）`,
    planNoteTaskSuccess: '任务成败判定的取证仲裁和判废护栏调用视数据而定，没有计入',
  ...
  report: {
    ...
    // Episodes and media (the Episode tab, the adjudication cards)
    ...
    perfLatencyNote: '失败：补发、重试之后仍没拿到结果的调用。耗时：第一次发出到最后一次返回的真实时长。',
    perfConcurrency: '有效并发度',
    perfConcurrencyFoot: 'Σ请求耗时 / VLM 段总耗时；接近 1 说明请求被串行化了',
    perfStages: '各阶段耗时对比',
    perfRetries: '重试与合并',
    outerAttempts: '外层重试',
    rescued: '挽回',
  ...
  /** The report page's frame (F6.2): tabs, header, integrity, collapsing. */
  reportPage: {
    tabEpisodes: 'Episode 明细',
    adjudicate: '人工裁决',
    adjudicateN: (n: number) => `人工裁决（${n}）`,
    adjudicateHint: '按 episode 逐条确认待裁决的条目，或复议被拒的条目',
    collapse: '收起',
    expand: '展开',
    collapseAll: '全部折叠',
    expandAll: '全部展开',
    durationParts: (parts: string[]) => parts.join(' + '),
    durationMain: '主流程',
    heldList: '出错的条目',
    oldReport: '这份报告生成于统计图上线之前，只有计数；之后生成的报告版本才有图。',
    integrity: {
      format: '格式',
      episodes: 'Episode',
      cameras: '相机',
    ...
    episodesValue: (count: number | null, scope: string) => (count === null ? scope : `数据集 ${count} 条，${scope}`),
    episodesAll: (n: number) => `本次全部 ${n} 条`,
    episodesHead: (n: number) => `本次前 ${n} 条（ep 0–${n - 1}）`,
    episodesExplicit: (expr: string, n: number) => `本次自选 ${expr}（${n} 条）`,
    episodesChecked: (n: number) => `本次质检 ${n} 条`,
    episodesSkipped: (n: number) => `；另有 ${n} 条缺源文件`,
    camerasValue: (names: string[]) => `${names.length} 路：${names.join('、')}`,
    fpsValue: (v: number) => `${Number(v.toFixed(2))} fps`,
    notRead: '未读到',
    robotKinematicsSkipped: '，运动学极限未运行',
    profileMatched: (name: string, by: string) => `命中 ${name}（按 ${by} 匹配）`,
    profileNone: '没有命中的档案：动作与状态的含义按数据本身推断',
    labelsValue: (withTask: number, without: number) => `${withTask} 条有；${without} 条没有`,
    labelsCaptioned: '，没有的由模型补描述（来源记为「自产描述」）',
    validationOk: '通过',
    none: '无',
    sourceValue: (objects: number, size: string, digest: string) => `${objects} 个对象 · ${size} · ${digest}（启动时固化）`,
    sourceDigest: (digest: string) => `${digest}（启动时固化）`,
    // preflight warnings (C2 preflight.warnings, English in the CLI) in Chinese
    warnMissingV2: (n: string, eps: string) => `${n} 条缺少 parquet 或某一路相机的视频（${eps}）：照 v1 的做法剔除，不参与质检、不进任何清单，名单见下表`,
    warnMissingV3: (n: string, eps: string) => `${n} 条缺少数据或视频文件（${eps}）：LeRobot v3 不剔除它们（v1 照读），读不到的在检查时算出错`,
  ...
  sections: {
    ...
    noStats: '这一节没有统计数据',
    noChart: '没有可画的数据',
    unknownModule: '这个模块没有专用视图，按通用方式列出它的统计',
    score: {
      mean: '平均分',
      hist: '分数分布',
      histDesc: '横轴是分数，纵轴是条数',
    },
    abstainReasons: '弃权原因',
    errorSteps: '出错停在哪一步',
    step: { probe: '打分', endstate: '复核', arbitration: '取证仲裁', caption: '打标', llm: '归纳', decode: '解码', read: '读数据', autolabel: '补任务描述', internal: '内部错误', guard_caption: '护栏打标', unknown: '未注明' } as Record<string, string>,
    timestamp: {
      failed: '不合格',
      kinds: { out_of_order: '乱序', gap: '跳变（丢帧）', fragment: '残段', jitter: '间隔抖动', other: '其它' } as Record<string, string>,
      failChart: '不合格原因',
      durationTotal: '总时长',
      durationMedian: '时长中位数',
      durationRange: '最短 / 最长',
      durationChart: '时长分布',
      durationDesc: '横轴是秒数，纵轴是条数',
      seconds: (v: number) => `${Number(v.toFixed(2))} 秒`,
    },
    kinematics: {
      episodes: '越限条数',
      profile: '规格档',
      byType: '按越限类型',
      byJoint: '按关节',
      byDesc: '同一条在同一类型 / 关节上只算一次',
      types: { joint_limit: '关节超限', velocity_limit: '关节超速', velocity: '超速', ee_reach: '末端超出可达范围', ee_translation_velocity: '末端平移超速', ee_rotation_velocity: '末端旋转超速', format_or_other: '格式 / 其它' } as Record<string, string>,
      joint: (j: string) => (/^\d+$/.test(j) ? `关节 ${j}` : ({ xyz: '末端位置', rpy: '末端姿态' } as Record<string, string>)[j] ?? j),
      none: '没有越限的条目',
    },
    motion: {
      stuck: '执行器卡死',
      stuckFoot: '单列，不进总分',
      stuckNa: (n: number) => `${n} 条无法评估`,
      activeRatio: '运动占比',
      activeFoot: '在动的时间 / 总时长',
      subscores: '子项均分',
      subscoresDesc: '蓝色计入总分，青色只报不罚；不适用的子项不画',
      sub: { smoothness: '平滑度', spike: '尖刺', gripper_jitter: '夹爪抖动', actuator_saturation: '执行器饱和', path_efficiency: '路径效率', joint_stability: '末态稳定', fluency: '流畅度', stuck: '卡顿' } as Record<string, string>,
      inTotal: '计入总分',
      reportOnly: '只报不罚',
    ...
    visual: {
      perCamera: (n: number) => `每条 ${n} 路相机`,
      low: '低于 0.6 的相机读数',
      placeholder: '占位（无画面）',
      blurRef: '清晰度参考线',
      maxSide: '画面最长边',
      maxSideFoot: (v: number) => `${v} 像素`,
      weights: '相机权重',
    ...
    sync: {
      ...
      verdictFoot: { annotated: '不判废', suspect: '证据不足，只提醒', undecidable: '弃权，不进裁决' } as Record<string, string>,
      flagged: '被标注的相机',
      flaggedUnit: '路次',
      verdictChart: '判定分布',
      cameraTable: '全库逐相机同步概览',
      cols: { camera: '相机', readings: '读数', trusted: '可信读数', median: '典型滞后（秒）', iqr: '逐条波动（秒）', suspect: '疑似错位', abstained: '测不准', flagged: '已标注' },
      lagChart: '逐相机典型滞后',
      lagChartDesc: (tol: number) => `正 = 画面比动作晚；绿带是容差 ±${tol} 秒`,
    ...
    task: {
      ...
      fail: '判废',
      abstain: '弃权（转人工）',
      abstainFoot: '先交付，同时等人确认',
      error: '出错（待补跑）',
      sources: '任务文本来源',
      sourceNames: { 原始标注: '原始标注', 自产caption: '自产描述', 人工改标: '人工改标' } as Record<string, string>,
      judgements: '判定结论分布',
      judgementsDesc: '每条最后落在哪个结论上',
      judgement: {
        success: '打分层判成功',
        recovery: '中途回落后完成',
        endstate_success: '复核救回',
        arbitration_success: '仲裁救回',
        failure: '判失败（双签）',
        arbitration_failure: '仲裁判失败',
        uncertain: '灰区，拿不准',
        gap_violation: '峰值后回落',
        voc_tripwire: '时序绊线',
        endstate_failure_suspect: '复核否决弱成功',
        endstate_unconfirmed: '弱成功未获印证',
        review_conflict: '两层证据矛盾',
        label_conflict_suspect: '疑似标注错（护栏拦下）',
        other: '其它',
      } as Record<string, string>,
      layers: '各层判定条数',
      layersDesc: '走到每一层的条数：越往后越少',
      layer: { probe: '打分层', endstate: '逐机位复核', label_guard: '判废护栏', arbitration: '取证仲裁' } as Record<string, string>,
      arbitration: '取证仲裁',
      arbitrationValue: (triggered: number, rescued: number, failed: number, abstained: number) => `触发 ${triggered} 条：救回 ${rescued} · 判废 ${failed} · 仍拿不准 ${abstained}`,
      method: '判定口径',
      methodLines: [
        '打分层：8 个时刻、各机位联合打完成度（0–100）。',
        '复核：每个机位分开问「做成了吗」「失败了吗」，一个机位一票。',
    ...
    dedup: {
      ...
      size: (n: string) => `${n} 条一组`,
      none: '没有发现重复的条目',
      note: '去重固定单线程：并发会让同一份数据两次跑出不同的保留对象。',
    },
    skill: {
      covered: '覆盖条数',
      coveredFoot: '去重后的交付条目',
      families: '技能族',
      subskills: '子技能',
      undersampled: '样本偏少的族',
      disagreements: '标注分歧',
      disagreementFoot: (high: number, review: number) => `高置信 ${high} · 人工复核 ${review}`,
      unstable: '看法不稳',
      unstableFoot: '不算分歧，只列出来',
      familyChart: '技能分布',
      familyChartDesc: '按技能族数条目',
      subskillChart: '子技能分布',
      subskillChartDesc: '族 › 子技能',
      groupingSources: '分组依据',
      groupingValue: (parts: string[]) => parts.join(' · '),
    },
    eef: {
      ...
      humanReason: {
        not_in_file: '文件里没有这一条',
        not_assessable: '位置无法评估',
        conflict: 'CPU 与模型意见相反',
        no_model_opinion: 'CPU 可疑但模型没给意见',
        model_cannot_see: 'CPU 可疑、模型看不了的分项',
        tracking_suspect: '模型认为独立观测跟错了目标',
      } as Record<string, string>,
      rejectChart: '判废来自哪些分项',
      agreement: '模型与 CPU 一致率',
      agreementFoot: (votes: number) => `按 ${votes} 张明确的票计`,
      overall: { candidates: '候选', assessed: '全部分项可评估', partially_assessable: '部分可评估', not_assessable: '无法评估', errors: '执行出错' } as Record<string, string>,
      overallChart: '总体评估',
      coverage: '可比覆盖率',
      coverageValue: (median: string, min: string) => `中位 ${median} · 最低 ${min}`,
      matrix: '分项状态',
      matrixDesc: '每个分项各有多少条落在每种状态',
      subitem: { position_2d: '位置', orientation_2d: '朝向', temporal_alignment: '时间对齐', state_motion: '状态运动', camera_motion: '相机运动', input_consistency: '输入一致性', vlm_review: 'VLM 复核' } as Record<string, string>,
      status: { ok: '正常', suspect: '可疑', unknown: '无法评估', unsupported: '不支持', error: '出错' } as Record<string, string>,
      hypotheses: '被支持的诊断假设',
      review: { reviewed: '完整复核', incomplete: '未完成', not_reviewed: '未复核', conflicts: '与 CPU 冲突的窗口', windows: '复核窗口', answered: '有答复', failed: '失败' },
      reviewClasses: { support: '支持', refute: '反驳', uncertain: '拿不准', not_observable: '看不到' } as Record<string, string>,
      windowsValue: (answered: number, failed: number) => `有答复 ${answered} · 失败 ${failed}`,
    },
  },
  ...
  episodeTab: {
    ...
    prev: '上一条',
    next: '下一条',
    list: { passed: '通过', reject: '拒绝', held: '待补跑' } as Record<string, string>,
    reviewBadge: '待裁',
    noMatch: '没有符合条件的 episode',
    empty: '这个版本没有 episode',
    notFiltered: '这一条不在当前筛选的范围里',
    summary: '判决概要',
    reasons: '判定原因',
    review: '待人工确认',
    goAdjudicate: '去裁决',
    taskText: '任务文本',
    taskSource: (s: string) => `（来源：${s}）`,
    videos: '各机位视频',
    evidence: '证据帧',
    notReached: '未进入这一档（前面已判废或待补跑）',
    noRecord: '这个模块没有这一条的读数',
    elapsed: '耗时',
    error: '执行出错',
    advisory: '建议性，不影响判决',
    timestamp: { duration: '时长', frames: '帧数', dtNominal: '名义帧间隔', maxDt: '最大帧间隔', jitter: '间隔抖动占比', gaps: '异常位置', gap: (frame: number, dt: string) => `第 ${frame + 1} 帧后隔了 ${dt} 秒`, stable: '帧间隔稳定' },
    kinematics: { none: '关节 / 末端全程在规格内', count: (n: number) => `${n} 处越限`, cols: { type: '类型', joint: '关节 / 轴', frame: '帧', value: '实测值', limit: '极限' }, more: (n: number) => `另有 ${n} 处未列出`, sustained: '持续在限外的关节', profile: '规格档' },
    motion: {
      cols: { name: '子项', value: '得分', role: '计分', note: '说明' },
      na: '不适用',
      stuck: '执行器卡死',
      stuckYes: (joints: string) => `卡死：${joints}`,
      stuckJoint: (axis: string, from: number, to: number) => `${axis}（第 ${from}–${to} 帧）`,
      stuckNo: '没有执行器卡死',
      idle: '空闲',
      idleValue: (head: string, tail: string, mid: number, midS: string) => `开头 ${head} 秒 · 结尾 ${tail} 秒 · 中途停顿 ${mid} 次（共 ${midS} 秒）`,
      active: '运动占比',
      sameSource: '指令与读数同源（指令即下一帧读数）：执行器饱和、卡顿无法评估，按不适用处理。',
    },
    visual: { cols: { camera: '相机', score: '视觉总分', sharpness: '清晰度', exposure: '曝光', integrity: '完整性', frozen: '冻结帧占比', status: '状态' }, ok: '正常', padded: '占位（无画面）', dead: '无画面', worst: (cam: string) => `最低的一路：${cam}` },
    sync: {
      badge: { aligned: '同步正常', annotated: '已标注异常（不判废）', suspect: '疑似错位（证据不足）', undecidable: '测不准（弃权）', misaligned_all: '整体错位（判废）' } as Record<string, string>,
      cols: { camera: '相机', lag: '滞后（秒）', peak: '相关峰值', zero: '零偏相关', trusted: '可信', diagnosis: '诊断' },
      consensus: (lag: string) => `各路可信相机一致滞后 ${lag} 秒`,
      curves: '同步曲线',
      curvesDesc: '左：画面运动与机械臂运动（各自归一化），同起同落才对；右：互相关曲线，峰落在绿带（0 ± 容差）内就是对齐',
      flow: '画面运动',
      speed: '机械臂运动',
      xcorr: '互相关',
      lagAxis: '滞后（秒，> 0 表示画面晚）',
      timeAxis: '时间（秒）',
      peak: (cam: string, lag: string, corr: string) => `${cam} 峰 ${corr} @ ${lag} 秒`,
    },
    task: {
      trail: '判定链',
      layers: { probe: '打分层', endstate: '逐机位复核', label_guard: '判废护栏', arbitration: '取证仲裁', final: '结论' } as Record<string, string>,
      notReached: '未触发',
      init: { success: '成功候选', recovery: '中途回落后完成', failure: '失败候选', uncertain: '拿不准', gap_violation: '峰值后回落', voc_tripwire: '时序绊线', score_blind: '无信息', no_progress: '全程无进展' } as Record<string, string>,
      strong: '（强）',
      weak: '（弱）',
      tally: { yes: '一致判完成', no: '一致判未完成', split: '复核分歧', abstain: '全体弃权' } as Record<string, string>,
      votes: (yes: number, no: number, other: number) => `完成 ${yes} · 未完成 ${no} · 看不清 ${other}`,
      guard: { same: '标注与画面一致，维持', different: '标注与画面不是同一件事，拦下', unavailable: '没有对照物，维持', error: '核不了，从严不判废' } as Record<string, string>,
      arbitration: (n: number, consensus: string) => `${n} 路取证，${consensus}`,
      consensus: { yes: '一致判完成', no: '一致判未完成', abstain: '拿不准' } as Record<string, string>,
      final: { pass: '通过', fail: '判废', abstain: '转人工', error: '出错' } as Record<string, string>,
      reason: '理由',
      intent: (s: string) => `判定用的任务文本来自${s}`,
      completion: '打分层完成度',
      completionDesc: '8 个时刻的完成度（0–1）',
      videoAssessment: '视频判定',
      videoCompletion: (score: string) => ` · 主判完成度 ${score}%`,
      videoEvidence: (camera: string, start: string, end: string) => `${camera} · ${start}–${end} 秒`,
