...
export const zh = {
  ...
  adjudication: {
    ...
    status: { pending: '待裁', decided: '已裁', unsure: '拿不准', applied: '已应用' } as Record<string, string>,
    ...
    optionalVerdictDesc: '你改了标注。给了成败结论就直接采用、不再重判；留空则按新标注重跑任务成败判定。',
    discard: '其它原因，整条弃用',
    undiscard: '撤销整条弃用',
    ...
    videos: '视频',
    // appeals
    appealsIntro: '这里只列任务成败判定拒掉的条目。看完视频，如果认为判定有误，可以把它恢复为可用。复议结论同样要点「执行裁决」才生效，它不计入「待裁」条数。',
    whyNotHere: '为什么有的被拒条目不在这里',
    whyNotHereBody: (total: number, others: number) => `本次一共拒绝 ${total} 条，另外 ${others} 条的结论不接受复议：`,
    whyNotHereRule: '时间戳、运动学、视频-动作同步量的是数据本身，去重比的是文件字节，复议也改变不了结论。只有任务成败判定是模型判的，才开放复议；后端同样会拒绝别的复议。',
  ...
  credentials: {
    ...
    addModel: '添加并验证',
    addModelPlaceholder: '请输入 Model ID 或推理接入点 ID（ep-…）',
